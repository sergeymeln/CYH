...
<div id="internetOffers2" class="tab-pane fade">
    <table class="table providers-table offers-table hidden-xs">
        ...
        foreach ($products as $prod) {
            if((!in_array($prod->ServiceProviderCategory->Category->Id,$constants['internetCats'])) ||
                $prod->ServiceProviderCategory->Provider->Id != $brandId){continue;}
            $tempCounter++;
        }
        ?>
        <?php if($tempCounter > 0):?>
            <?php foreach($products as $prod):?>
                <?php if((!in_array($prod->ServiceProviderCategory->Category->Id,$constants['internetCats'])) ||
                    $prod->ServiceProviderCategory->Provider->Id != $brandId){continue;}?>
                <?php
...
<div id="internetTvOffers2" class="tab-pane fade">
    <table class="table providers-table offers-table hidden-xs">
        ...
        <?php $tempCounter = 0;
        foreach ($products as $prod) {
            if(!in_array($prod->ServiceProviderCategory->Category->Id,$constants['internetAndTvCats']) ||
                $prod->ServiceProviderCategory->Provider->Id != $brandId){continue;}
            $tempCounter++;
        }
        ?>
        <?php if($tempCounter > 0):?>
            <?php foreach($products as $prod):?>
                <?php if((!in_array($prod->ServiceProviderCategory->Category->Id,$constants['internetAndTvCats'])) ||
                    $prod->ServiceProviderCategory->Provider->Id != $brandId) {continue;}?>
                <?php
                $speedData = \CYH\Marketing\Helpers\ProductDataHelper::getSpeedData($prod->DownloadSpeed);
                $content = \CYH\Helpers\ContentDeserializeHelper::GetDescriptionFromTags($prod->Description);
