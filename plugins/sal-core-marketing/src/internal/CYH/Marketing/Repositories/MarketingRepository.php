...
class MarketingRepository
{
    ...
    public function GetPublishedCities($isSecondBatch=false)
    {
        ...
        $select->where->in('city_type', self::CITIES_IN);
        if ($isSecondBatch === true) {
            $select->where->notIn('city_id', explode(',',self::PROD_IDS));
        } else {
            $select->where->in('city_id', explode(',',self::PROD_IDS));
        }
     ...
     * @return array
     */
    public function getNearestPublishedCity($cityData, CacheSettings $cacheSettings, $radius=1500)
    {
        $cacheKey = $this->_cacheProvider->GetCacheKey(__CLASS__, __FUNCTION__, [$cityData['city_normal_name'], $cityData['state_code'], $radius]);
        $cachedData = [];
